...
string toString(FrameHeader.OpCode oc) {
	final switch (oc) {
		case FrameHeader.OpCode.ERROR: return "ERROR";
		case FrameHeader.OpCode.STARTUP: return "STARTUP";
		case FrameHeader.OpCode.READY: return "READY";
		case FrameHeader.OpCode.AUTHENTICATE: return "AUTHENTICATE";
		case FrameHeader.OpCode.CREDENTIALS: return "CREDENTIALS";
		case FrameHeader.OpCode.OPTIONS: return "OPTIONS";
		case FrameHeader.OpCode.SUPPORTED: return "SUPPORTED";
		case FrameHeader.OpCode.QUERY: return "QUERY";
		case FrameHeader.OpCode.RESULT: return "RESULT";
		case FrameHeader.OpCode.PREPARE: return "PREPARE";
		case FrameHeader.OpCode.EXECUTE: return "EXECUTE";
		case FrameHeader.OpCode.REGISTER: return "REGISTER";
		case FrameHeader.OpCode.EVENT: return "EVENT";
	}
}
...
FrameHeader readFrameHeader(Socket s, ref int counter) {
...
void printFrameHeader(FrameHeader fh) {
	writefln("========    FRAME HEADER    =======\n======== ======== ========= =======\n%8b     %8b %8b\n   %32d\n===================================", fh.version_,
	fh.streamid,
	fh.opcode,
	fh.length);
}

byte readByte(Socket s, ref int counter) {
	byte[1] buf;
 ...
 */
int* readInt(ref int ptr, Socket s, ref int counter) {
	import std.bitmanip : read;
	ubyte[int.sizeof] buffer;
	auto n = s.receive(buffer);
	assert(n==int.sizeof);
...
int readIntNotNULL(ref int ptr, Socket s, ref int counter) {
	auto tmp = readInt(ptr, s, counter);
	if (tmp is null) throw new Exception("NullException");
	return *tmp;
}
void write(Socket s, int n) {
	import std.bitmanip : write;
...
short readShort(Socket s, ref int counter) {
	import std.bitmanip : read;
	ubyte[short.sizeof] buffer;
	auto n = s.receive(buffer);
	assert(n==short.sizeof);
 ...
 *                   string.
 */
ubyte[] readRawBytes(Socket s, ref int counter, int len) {
...
string readShortString(Socket s, ref int counter) {
...
void write(Socket s, string str) {
...
string readLongString(Socket s, ref int counter) {
	int len;
...
StringList readStringList(Socket s, ref int counter) {
	StringList ret;
	auto len = readShort(s, counter);
 ...
 */
ubyte[] readIntBytes(Socket s, ref int counter) {
	int len;
	auto tmp = readInt(len, s, counter);
	if (tmp is null) {
 ...
 */
ubyte[] readShortBytes(Socket s, ref int counter) {
	auto len = readShort(s, counter);
	if (len==0) { return null; }
...
/+
/// This implementation has problems with types T where T.init is null
template hasUFCSmember(T, string member) {
	/* pragma(msg, "p1[", T, "] ", is (ParameterTypeTuple!toBytes[0] == T));
	 pragma(msg, "compiles: ", __traits(compiles, toBytes()));
	 pragma(msg, "check: ", is(typeof(&T.toBytes) == ubyte[] function(T)));
	 pragma(msg, "check2: ", "__traits(compiles, (", T, ").toBytes))", __traits(compiles, (T.init).toBytes));*/
	 enum hasUFCSmember = __traits(compiles, mixin("T v;"~member ~"(v)")) ? true : false;
}+/

/// returns the overload that matches a function making sure no implicit casts are used to resolve the function to call
/+template callExact(string func, string modulename = __MODULE__, T) {
	static if (result) {
		pragma(msg, "isBasicType!T: ", isBasicType!T);
		static if (!isBasicType!T) {
			enum modulename = moduleName!T;
		}
		pragma(msg, "modulename: ", modulename);
		foreach (f; __traits(getOverloads, modulename, member)) {
			static if (is (ParameterTypeTuple!f[0] == T) {
				return f;
			}
		}
	}
...
auto appendIntRawBytes(T)(Appender!(ubyte[]) appender, T data) {
	static if (is(T == string) || is(T == ubyte[]) || is(T == byte[])) {
		...
		appender.appendIntRawBytes(tmpapp.data);
...
StringMultiMap readStringMultiMap(Socket s, ref int counter) {
...
class Connection {
	...
	private void write(Socket s, Appender!(ubyte[]) appender) {
		print(appender.data);
		if (s.send(appender.data) != appender.data.length) {
			throw new Exception("Write exception, not all appender was written");
		}
	}


	//vvvvvvvvvvvvvvvvvvvvv CQL Implementation vvvvvvvvvvvvvvvvvv
	int counter; // keeps track of data left after the header
	...
	FrameHeader makeHeader(FrameHeader.OpCode opcode) {
		FrameHeader fh;
		fh.version_ = transport_version_;
	 ...
	 */
	 void startup(string compression_algorithm = "") {
		...
		write(sock, appender!(ubyte[])().append(bytes!fh));
		write(sock, bytebuf);
	...
	private auto execute(Args...)(ubyte[] preparedStatementID, Consistency consistency, Args args) {
		...
		throwOnError(fh);
		if (!fh.isRESULT) {
			throw new Exception("CQLProtocolException, Unknown response to Execute command: "~ .toString(fh.opcode));
		}
		return new Result(fh);
	}
	...
	void listen(Event events...) {
		...
		auto bytebuf = appender!(ubyte[])();
		bytebuf.put(bytes!fh);
		assert(false);
		writeEvent(sock, events);
	...
	void throwOnError(FrameHeader fh) {
		...
		final switch (code) {
			...
			case Error.UnavailableException:
				...
				throw new Exception("CQL Exception, "~ spec_msg ~ msg ~" consistency:"~ .toString(cs) ~" required:"~ to!string(required) ~" alive:"~ to!string(alive));
			...
			case Error.WriteTimeout:
				...
				throw new Exception("CQL Exception, "~ spec_msg ~ msg ~" consistency:"~ .toString(cl) ~" received:"~ to!string(received) ~" blockfor:"~ to!string(blockfor) ~" writeType:"~ toString(writeType));
			case Error.ReadTimeout:
				auto cl = cast(Consistency)readShort(sock,counter);
				auto received = readIntNotNULL(tmp, sock, counter);
				auto blockfor = readIntNotNULL(tmp, sock, counter); // WARN: the type for blockfor does not seem to be in the spec!!!
				auto data_present = readByte(sock, counter);
				throw new Exception("CQL Exception, "~ spec_msg ~ msg ~" consistency:"~ .toString(cl) ~" received:"~ to!string(received) ~" blockfor:"~ to!string(blockfor) ~" data_present:"~ (data_present==0x00?"false":"true"));
	...
	class Result {
		 ...
		 */
		struct MetaData {
			int flags; immutable GLOBAL_TABLES_SPEC = 0x0001;
			int columns_count;
		 ...
		 */
		Option* readOption(Socket s, ref int counter) {
			auto ret = new Option();
	...
	void writeEvent(Socket s, Event e) {
		.write(s, cast(string)e);
	}
...
unittest {
	auto address = getAddress("127.0.0.1", 9042);

	auto cassandra = new Connection();
	cassandra.connect(address);
...
/+
	try {
		writefln("QUERY");
		cassandra.query("USE twissandra", Consistency.ANY);
...
/+
	...
	try {
		...
		assert(cassandra.insert(`INSERT INTO alltypes (user_name,birth_year,ascii_col,blob_col,booleant_col, booleanf_col,decimal_col,double_col,float_col,inet_col,int_col,list_col,map_col,set_col,text_col,timestamp_col,uuid_col,timeuuid_col,varint_col)
				VALUES ('bob@domain.com', 7777777777,
					'someasciitext', '2020202020202020202020202020',
					True, False,
	...
	/*try {
		...
		auto stmt = cassandra.prepare(`INSERT INTO alltypes (user_name,birth_year,ascii_col,blob_col,booleant_col, booleanf_col
		,decimal_col,double_col,float_col,inet_col,int_col,list_col,map_col,set_col,text_col,timestamp_col
		,uuid_col,timeuuid_col,varint_col)`
				` VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
		writeln(stmt);
		stmt.execute("rory",1212121212, "mossesf asciiiteeeext", 0x898989898989, true, false, 999, 8.88
			, 7.77, "192.168.0.1", 66666, ["one","two"], "{\"key1\":\"value1\"}", "[\"one\", \"two\", \"three\"]"
			, "some more text«»", "2013-08-31", "", "", 5555555);
	} catch (Exception e) {writefln(e.msg);}*/
