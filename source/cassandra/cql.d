...
class Connection {
	...
	PreparedStatement prepare(string q) {
		...
		return new PreparedStatement(this, fh, sock, m_counter);
	}
...
class PreparedStatement : CassandraResult {
	...
	this(Connection conn, FrameHeader fh, TCPConnection sock, ref int counter)
	{
		m_conn = conn;
	...
	CassandraResult execute(Args...)(Args args)
	{
		return m_conn.execute(m_id, m_consistency, args);
	}
...
unittest {
	...
	try {
		...
		writefln("using %s %s", res.kind, res.keyspace);
	} catch (Exception e) {
		...
		try {
			...
			writefln("created %s %s %s", res.kind, res.keyspace, res.lastchange);
	...
	try {
		...
		writefln("created table %s %s %s %s", res.kind, res.keyspace, res.lastchange, res.table);
	} catch (Exception e) {writefln(e.msg);}
	...
	try {
		...
		writeln(rows[0]);
		writefln("select resulted in CassandraResult.Kind.%s\n%s", res.kind, res);
	} catch (Exception e) {writeln(e);}
